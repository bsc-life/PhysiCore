...
#pragma omp for
		for (index_t i = 0; i < count; ++i)
		{
			...
			real_t persistence_time = mech_data.persistence_times[i];
			real_t migration_speed = mech_data.migration_speeds[i];
			real_t migration_bias = mech_data.migration_biases[i];
			...
			if (persistence_time > 0.0 && uniform(gen) < dt / persistence_time)
			{
				...
				real_t theta = 2.0 * M_PI * uniform(gen);
				real_t phi = std::acos(2.0 * uniform(gen) - 1.0);

				real_t rand_x = std::sin(phi) * std::cos(theta);
				real_t rand_y = std::sin(phi) * std::sin(theta);
				real_t rand_z = std::cos(phi);

				// Get bias direction
				real_t bias_x = mech_data.migration_bias_directions[i * 3];
				real_t bias_y = mech_data.migration_bias_directions[i * 3 + 1];
				real_t bias_z = mech_data.migration_bias_directions[i * 3 + 2];
				...
				// Normalize
				real_t mag = std::sqrt(dir_x * dir_x + dir_y * dir_y + dir_z * dir_z);
				if (mag > 1e-16)
				{
					dir_x /= mag;
					dir_y /= mag;
