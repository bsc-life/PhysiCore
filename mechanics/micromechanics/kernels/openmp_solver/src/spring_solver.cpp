...
void spring_solver::update_spring_attachments(environment& e)
{
	...
	real_t elastic_constant = e.params.attachment_elastic_constant;
	real_t attachment_rate = e.params.attachment_rate;
	real_t detachment_rate = e.params.detachment_rate;
...
#pragma omp parallel for
	for (index_t i = 0; i < count; ++i)
	{
		...
		for (index_t j : attachments)
		{
			...
			real_t rest_length = mech_data.radii[i] + mech_data.radii[j];

			// Spring force: F = k * (distance - rest_length)
			// Positive when stretched (pulls together), negative when compressed
			real_t force_mag = elastic_constant * (distance - rest_length);

			// Normalize direction
			real_t nx = dx / distance;
			real_t ny = dy / distance;
			real_t nz = dz / distance;
