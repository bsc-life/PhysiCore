...
#pragma omp for
		for (index_t i = 0; i < data.base_data.agents_count; i++)
		{
			...
			compute_internalized(
				data.internalized_substrates.data() + i * data.substrate_count, substrates,
				data.get<secretion_uptake_component>()->secretion_rates.data() + i * data.substrate_count,
				data.get<secretion_uptake_component>()->uptake_rates.data() + i * data.substrate_count,
				data.get<secretion_uptake_component>()->saturation_densities.data() + i * data.substrate_count,
				data.get<net_export_component>()->net_export_rates.data() + i * data.substrate_count, data.volumes[i],
				voxel_volume, timestep, dens_l ^ fixed_dims);
		}
	}
}
			  ...
			  std::atomic<bool>* HWY_RESTRICT is_conflict)
{
	if (recompute)
	{
		compute_intermediates(
			numerators, denominators, factors, data.get<secretion_uptake_component>()->secretion_rates.data(),
			data.get<secretion_uptake_component>()->uptake_rates.data(),
			data.get<secretion_uptake_component>()->saturation_densities.data(),
			data.get<net_export_component>()->net_export_rates.data(), data.volumes.data(),
