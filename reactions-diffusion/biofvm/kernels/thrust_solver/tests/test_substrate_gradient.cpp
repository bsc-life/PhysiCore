...
std::unique_ptr<microenvironment> gradient_test_microenv()
{
	...
	builder.select_solver(PHYSICORE_THRUST_SOLVER_REGISTRY_NAME);
...
TEST(PREPEND_TEST_NAME(SubstrateGradientTest), CentralDifferenceAtInteriorVoxel)
{
...
TEST(PREPEND_TEST_NAME(SubstrateGradientTest), OneSidedDifferenceAtMinBoundary)
{
	auto env = gradient_test_microenv();
...
TEST(PREPEND_TEST_NAME(SubstrateGradientTest), OneSidedDifferenceAtMaxBoundary)
{
	auto env = gradient_test_microenv();
